...
filename_metabolic_model = 'E:\zyx\rmta_fastcore/Data/exampleData/Recon2.v04.mat';
model = load(filename_metabolic_model)
...
colnames = onmic_data.Properties.VariableNames(2:end);
fpkm = table2array(onmic_data(:,2:end));
...
rmRXNS = model.rxns(min>-1e-6 & max<1e-6)
% Cmodel = removeRxns(model, model.rxns(setdiff(1:numel(model.rxns),A)))
Cmodel = removeRxns(model,rmRXNS)
[~, idx] = ismember(Cmodel.rxns, model.rxns);
Cmodel.rev = model.rev(idx);

% A = FastMM_FVA_cplex(model, 1e-4,0)
% 
% rmRxns = model.rxns(A(:,1)>-1e-6 & A(:,2)<1e-6)
% Cmodel = removeRxns(model,rmRxns)
...
load medium_example % modify thih when needed
%% 
% In the original formulation of the paper and in our implementation, the 
% algorithm selected for tissue reconstruction is iMAT. iMAT is computationally 
% expensive and it's solution is not unique, so we provide the result of iMAT 
...
[logic_loc, loc] = ismember(dico.SYMBOL,differ_genes.Row);
differ_genes = differ_genes(ismember(differ_genes.Row,dico.SYMBOL(logic_loc)),:);
differ_genes.ENTREZ_ID = dico.ENTREZ(logic_loc);
...
differ_genes.logFC = differ_genes.log2FoldChange;
...
% execute the code
% this model can be changed to normal model which is built by control
% transcript data
tic
...
save(['Data' filesep 'cancer_case.mat'])
%% 
...
rMTAsaveInExcel(['Data' filesep 'cancer_case.xlsx'], TSscore, deletedGenes, alpha_values, ...
